...
def test_generate_control_dict_writes_expected_file(tmp_path) -> None:
...
def test_generate_transport_properties_includes_viscosity(tmp_path) -> None:
...
def test_generate_turbulence_properties_ras_model(tmp_path) -> None:
...
def test_generate_turbulence_properties_laminar(tmp_path) -> None:
    generate_turbulence_properties(tmp_path, turbulence_model="laminar")
    content = (tmp_path / "constant" / "turbulenceProperties").read_text()
    assert "simulationType  laminar;" in content


def test_generate_velocity_field_resolves_angle_of_attack(tmp_path) -> None:
...
def test_generate_velocity_field_nonzero_aoa_matches_trig(tmp_path) -> None:
...
def test_generate_pressure_field_writes_expected_file(tmp_path) -> None:
...
def test_openfoam_case_generate_creates_all_expected_files(tmp_path) -> None:
...
def test_openfoam_case_run_raises_without_generate(tmp_path) -> None:
    case = OpenFOAMCase("test_case", base_dir=tmp_path)
    with pytest.raises(InvalidCaseError):
        case.run()


def test_openfoam_case_run_raises_not_found_when_generated_but_no_openfoam(tmp_path) -> None:
    case = OpenFOAMCase("test_case", base_dir=tmp_path)
    case.generate()
...
def test_parse_force_coefficients_reads_last_row(tmp_path) -> None:
...
def test_parse_force_coefficients_raises_without_postprocessing_dir(tmp_path) -> None:
    with pytest.raises(InvalidCaseError):
        parse_force_coefficients(tmp_path)


def test_parse_force_coefficients_raises_on_empty_data(tmp_path) -> None:
    out_dir = tmp_path / "postProcessing" / "forceCoeffs1" / "0"
    out_dir.mkdir(parents=True)
...
def test_openfoam_case_postprocess_integrates_with_parser(tmp_path) -> None:
    case = OpenFOAMCase("test_case", base_dir=tmp_path)
